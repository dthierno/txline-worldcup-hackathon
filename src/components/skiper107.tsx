"use client";

import { useMemo, useState } from "react";
import { motion } from "motion/react";

import { cn } from "@/lib/utils";

export type Team = { name: string; code: string };
export type MatchSide = { team: Team | null; score: string | null };
export type Match = {
  id: string;
  date: string;
  status: "finished" | "upcoming";
  home: MatchSide;
  away: MatchSide;
  winner?: "home" | "away";
  penalties?: boolean;
};
export type Round = { name: string; matches: Match[] };

const CARD_W = 250;
const CARD_H = 124;
const GAP_X = 40;
const GAP_Y = 32;
const PITCH = CARD_H + GAP_Y;
const VISIBLE_COLS = 3;
const TRANSITION = { duration: 0.5, ease: [0.22, 1, 0.36, 1] as const };

const side = (name: string, code: string, score: string | null): MatchSide => ({
  team: { name, code },
  score,
});
const tbd = (): MatchSide => ({ team: null, score: null });
const match = (
  id: string,
  date: string,
  home: MatchSide,
  away: MatchSide,
  winner?: "home" | "away",
  penalties = false,
): Match => ({
  id,
  date,
  home,
  away,
  winner,
  penalties,
  status: winner ? "finished" : "upcoming",
});

export const ROUNDS: Round[] = [
  {
    name: "Round of 32",
    matches: [
      match("r32-1", "Mon, 29 Jun", side("South Africa", "za", "0"), side("Canada", "ca", "1"), "away"),
      match("r32-2", "Tue, 30 Jun", side("Netherlands", "nl", "1 (2)"), side("Morocco", "ma", "1 (3)"), "away", true),
      match("r32-3", "Tue, 30 Jun", side("Germany", "de", "1 (3)"), side("Paraguay", "py", "1 (4)"), "away", true),
      match("r32-4", "Wed, 1 Jul", side("France", "fr", "3"), side("Sweden", "se", "0"), "home"),
      match("r32-5", "Thu, 2 Jul", side("Belgium", "be", "3"), side("Senegal", "sn", "2"), "home"),
      match("r32-6", "Thu, 2 Jul", side("USA", "us", "2"), side("Bosnia and Herzegovina", "ba", "0"), "home"),
      match("r32-7", "Fri, 3 Jul", side("Spain", "es", "3"), side("Austria", "at", "0"), "home"),
      match("r32-8", "Fri, 3 Jul", side("Portugal", "pt", "2"), side("Croatia", "hr", "1"), "home"),
      match("r32-9", "Mon, 29 Jun", side("Brazil", "br", "2"), side("Japan", "jp", "1"), "home"),
      match("r32-10", "Tue, 30 Jun", side("Côte d'Ivoire", "ci", "1"), side("Norway", "no", "2"), "away"),
      match("r32-11", "Wed, 1 Jul", side("Mexico", "mx", "2"), side("Ecuador", "ec", "0"), "home"),
      match("r32-12", "Wed, 1 Jul", side("England", "gb-eng", "2"), side("DR Congo", "cd", "1"), "home"),
      match("r32-13", "Fri, 3 Jul", side("Switzerland", "ch", "2"), side("Algeria", "dz", "0"), "home"),
      match("r32-14", "Sat, 4 Jul", side("Colombia", "co", "1"), side("Ghana", "gh", "0"), "home"),
      match("r32-15", "Fri, 3 Jul", side("Australia", "au", "1 (2)"), side("Egypt", "eg", "1 (4)"), "away", true),
      match("r32-16", "Sat, 4 Jul", side("Argentina", "ar", "3"), side("Cabo Verde", "cv", "2"), "home"),
    ],
  },
  {
    name: "Round of 16",
    matches: [
      match("r16-1", "Sat, 4 Jul", side("Canada", "ca", "0"), side("Morocco", "ma", "3"), "away"),
      match("r16-2", "Sun, 5 Jul", side("Paraguay", "py", "0"), side("France", "fr", "1"), "away"),
      match("r16-3", "Mon, 6 Jul", side("USA", "us", "1"), side("Belgium", "be", "4"), "away"),
      match("r16-4", "Mon, 6 Jul", side("Portugal", "pt", "0"), side("Spain", "es", "1"), "away"),
      match("r16-5", "Mon, 6 Jul", side("Brazil", "br", "1"), side("Norway", "no", "2"), "away"),
      match("r16-6", "Mon, 6 Jul", side("Mexico", "mx", "2"), side("England", "gb-eng", "3"), "away"),
      match("r16-7", "Tue, 7 Jul", side("Switzerland", "ch", "0 (4)"), side("Colombia", "co", "0 (3)"), "home", true),
      match("r16-8", "Tue, 7 Jul", side("Argentina", "ar", "3"), side("Egypt", "eg", "2"), "home"),
    ],
  },
  {
    name: "Quarter-finals",
    matches: [
      match("qf-1", "Fri, 10 Jul, 4:00 am", side("France", "fr", "2"), side("Morocco", "ma", "0"), "home"),
      match("qf-2", "Sat, 11 Jul, 3:00 am", side("Spain", "es", "2"), side("Belgium", "be", "1"), "home"),
      match("qf-3", "Sun, 12 Jul, 5:00 am", side("Norway", "no", null), side("England", "gb-eng", null)),
      match("qf-4", "Sun, 12 Jul, 10:00 am", side("Argentina", "ar", null), side("Switzerland", "ch", null)),
    ],
  },
  {
    name: "Semi-finals",
    matches: [
      match("sf-1", "Wed, 15 Jul, 4:00 am", side("France", "fr", null), side("Spain", "es", null)),
      match("sf-2", "Thu, 16 Jul, 3:00 am", tbd(), tbd()),
    ],
  },
  {
    name: "Final",
    matches: [match("f-1", "Mon, 20 Jul, 3:00 am", tbd(), tbd())],
  },
];

function ChevronIcon({ direction }: { direction: "left" | "right" }) {
  return (
    <svg
      aria-hidden="true"
      className="h-5 w-5"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      strokeWidth="2"
    >
      <path d={direction === "left" ? "m13 17-5-5 5-5m5 10-5-5 5-5" : "m6 7 5 5-5 5m5-10 5 5-5 5"} strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

function ShieldIcon() {
  return (
    <svg aria-hidden="true" className="h-5 w-5 fill-current text-muted-foreground/50" viewBox="0 0 24 24">
      <path d="M12 2 4 5v6c0 5.05 3.41 9.76 8 11 4.59-1.24 8-5.95 8-11V5l-8-3Z" />
    </svg>
  );
}

function TeamRow({ side, winner, finished }: { side: MatchSide; winner: boolean; finished: boolean }) {
  const muted = finished && !winner;
  return (
    <div className="flex items-center gap-3">
      {side.team ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          alt={`${side.team.name} flag`}
          className="h-5 w-7 rounded-[4px] border border-border/40 object-cover"
          draggable={false}
          loading="lazy"
          src={`https://flagcdn.com/w80/${side.team.code}.png`}
        />
      ) : (
        <span className="flex h-5 w-7 items-center justify-center"><ShieldIcon /></span>
      )}
      <span className={cn("flex-1 truncate text-base font-medium", muted && "text-muted-foreground")}>
        {side.team?.name ?? "TBD"}
      </span>
      {side.score !== null ? (
        <span className={cn("text-base font-medium tabular-nums", muted && "text-muted-foreground")}>
          {side.score}
        </span>
      ) : null}
      <span className={cn("border-y-4 border-r-[6px] border-y-transparent border-r-transparent", winner && "border-r-foreground")} />
    </div>
  );
}

function MatchCard({ match: game }: { match: Match }) {
  const finished = game.status === "finished";
  return (
    <div className="h-[124px] w-[250px] rounded-2xl border border-border/60 bg-muted p-4 transition-colors hover:border-border">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-sm leading-5 text-muted-foreground">{game.date}</span>
        {finished ? (
          <span className="rounded-full bg-muted px-2.5 text-xs font-medium leading-5 text-muted-foreground">
            {game.penalties ? "FT (P)" : "FT"}
          </span>
        ) : null}
      </div>
      <div className="space-y-2.5">
        <TeamRow side={game.home} winner={game.winner === "home"} finished={finished} />
        <TeamRow side={game.away} winner={game.winner === "away"} finished={finished} />
      </div>
    </div>
  );
}

function yFor(roundIndex: number, matchIndex: number, start: number) {
  if (roundIndex >= start) {
    const scale = 2 ** (roundIndex - start);
    return (scale * (matchIndex + 0.5) - 0.5) * PITCH;
  }
  const parent = Math.floor(matchIndex / 2 ** (start - roundIndex));
  return parent * PITCH;
}

export function KnockoutBracket({
  rounds = ROUNDS,
  initialRound = 0,
  className,
}: {
  rounds?: Round[];
  initialRound?: number;
  className?: string;
}) {
  const maxStart = Math.max(0, rounds.length - 2);
  const [start, setStart] = useState(Math.min(Math.max(0, initialRound), maxStart));
  const height = rounds[start].matches.length * PITCH - GAP_Y;
  const cards = useMemo(
    () => rounds.flatMap((round, ri) => round.matches.map((game, mi) => ({ round, game, ri, mi }))),
    [rounds],
  );

  return (
    <div className={cn("w-full max-w-[850px]", className)}>
      <div className="relative mb-6 h-10 overflow-hidden">
        <button
          type="button"
          aria-label="Previous round"
          disabled={start === 0}
          onClick={() => setStart((value) => Math.max(0, value - 1))}
          className="absolute left-0 top-0 z-10 flex h-10 w-10 items-center justify-center rounded-full text-foreground transition-colors hover:bg-muted disabled:pointer-events-none"
        >
          <motion.span animate={{ opacity: start === 0 ? 0 : 1 }} transition={TRANSITION}>
            <ChevronIcon direction="left" />
          </motion.span>
        </button>
        <button
          type="button"
          aria-label="Next round"
          disabled={start === maxStart}
          onClick={() => setStart((value) => Math.min(maxStart, value + 1))}
          className="absolute right-0 top-0 z-10 flex h-10 w-10 items-center justify-center rounded-full text-foreground transition-colors hover:bg-muted disabled:pointer-events-none"
        >
          <motion.span animate={{ opacity: start === maxStart ? 0 : 1 }} transition={TRANSITION}>
            <ChevronIcon direction="right" />
          </motion.span>
        </button>
        {rounds.map((round, ri) => {
          const visible = ri >= start && ri < start + VISIBLE_COLS;
          return (
            <motion.div
              key={round.name}
              className="absolute top-0 flex h-10 w-[250px] items-center justify-center"
              animate={{ x: 20 + (ri - start) * (CARD_W + GAP_X), opacity: visible ? 1 : 0 }}
              transition={TRANSITION}
            >
              <span className="text-base font-medium">{round.name}</span>
            </motion.div>
          );
        })}
      </div>

      <motion.div className="relative overflow-hidden" animate={{ height }} transition={TRANSITION}>
        {rounds[start].matches.map((_, index) => (
          <motion.span
            key={`entry-${index}`}
            className="absolute left-0 top-0 h-px w-5 bg-border/60"
            animate={{ y: index * PITCH + CARD_H / 2, opacity: 1 }}
            transition={TRANSITION}
          />
        ))}

        {rounds.slice(1).flatMap((round, offset) => {
          const ri = offset + 1;
          return round.matches.map((_, index) => {
            const prevRound = ri - 1;
            const x = 20 + (prevRound - start) * (CARD_W + GAP_X) + CARD_W;
            const y1 = yFor(prevRound, index * 2, start) + CARD_H / 2;
            const y2 = yFor(prevRound, index * 2 + 1, start) + CARD_H / 2;
            const visible = prevRound >= start && ri < start + VISIBLE_COLS;
            return (
              <motion.div
                key={`connector-${ri}-${index}`}
                className="absolute left-0 top-0 w-5 rounded-r-xl border-y border-r border-border/60"
                animate={{ x, y: y1, height: Math.max(0, y2 - y1), opacity: visible ? 1 : 0 }}
                transition={TRANSITION}
              >
                <span className="absolute left-full top-1/2 h-px w-5 bg-border/60" />
              </motion.div>
            );
          });
        })}

        {cards.map(({ game, ri, mi }) => {
          const visible = ri >= start && ri < start + VISIBLE_COLS;
          return (
            <motion.div
              key={game.id}
              className="absolute left-0 top-0"
              animate={{
                x: 20 + (ri - start) * (CARD_W + GAP_X),
                y: yFor(ri, mi, start),
                opacity: visible ? 1 : 0,
              }}
              transition={TRANSITION}
            >
              <MatchCard match={game} />
            </motion.div>
          );
        })}
      </motion.div>

      <div className="mt-10 border-t border-border/60 pt-6">
        <p className="text-sm italic text-muted-foreground">
          All times are in UTC+8 · Current time:{" "}
          <time className="tabular-nums not-italic">Mon, Jul 13, 2026, 12:57:33 AM</time>
        </p>
      </div>
    </div>
  );
}

export function Skiper107() {
  return <KnockoutBracket rounds={ROUNDS} initialRound={0} />;
}
